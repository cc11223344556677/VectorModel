...
class Topic:
    def __init__(self, topic_id: int, vector: np.ndarray, is_static: bool = False):
...
class TopicSpace:
    def __init__(self, 
        ...
        for i in range(n_static_topics, n_static_topics + n_dynamic_topics):
            vec = self.rng.standard_normal(n_dims)
            vec = vec / np.linalg.norm(vec)
            self.topics.append(Topic(i, vec, is_static=False))
    ...
    def replace_topic(self, topic_id: int):
        ...
        self.topics[topic_id] = Topic(topic_id, new_vector, is_static=False)
...
class Message():
    ...
    - a subset of attitude vectors indexed by attitude vector id
    ...
    def __init__(self, 
                 ...
                 att_vecs: List[Tuple[int, np.ndarray]] = None,
                 pers_vecs: List[Tuple[int, np.ndarray]] = None, 
                 target_ids: List[int] = None,
                 ):
        self.sender_id = sender_id
        self.opinions = opinions
        #TODO: Change name of attitude and find new name for pers_vecs
        self.att_vecs = att_vecs
...
class MessageSelector:
    ...
    'select_from_similar_personalities_ummatched',
    'select_from_similar_attitudes_matched',
    'select_from_similar_attitudes_unmatched',
    ]
    ...
    def __init__(
        ...
        if max_messages is not None and rng is None:
        ...
        for m in methods:
            match m:
                ...
                case 'select_from_similar_personalities_ummatched':
                    if vector_similarity_threshold is None:
                        raise ValueError(
                            f"method='{m}' requires vector_similarity_threshold"
                        )
                ...
                case 'select_from_similar_attitudes_matched':
                    if opinion_similarity_threshold is None:
                        raise ValueError(
                            f"method='{m}' requires vector_similarity_threshold"
                        )
                    self.vector_similarity_threshold = vector_similarity_threshold
                    self.selectors.append(self.select_from_similar_attitudes_matched)
                    
                case 'select_from_similar_attitudes_unmatched':
                    if opinion_similarity_threshold is None:
                        raise ValueError(
                            f"method='{m}' requires vector_similarity_threshold"
                        )
                    self.vector_similarity_threshold = vector_similarity_threshold
                    self.selectors.append(self.select_from_similar_attitudes_unmatched)
    ...
    def select_randomly(self, message_space: List[Message], agent: VectorAgent) -> List[Message]:
        ...
        return self.rng.choice(message_space, self.max_messages, replace=False)
    ...
    def select_from_similar_attitudes_matched(self, message_space: List[Message], agent: VectorAgent) -> List[Message]:
        messages = []
        
        for message in message_space:
            if not message.att_vecs: 
                continue 
            for vec_id, att_vec in message.att_vecs:
                if 1 - self.calculate_similarity(agent.att_vecs[vec_id], att_vec) < self.vector_similarity_threshold:
                    messages.append(message)
                    break
    ...
    def select_from_similar_attitudes_unmatched(self, message_space: List[Message], agent: VectorAgent) -> List[Message]:
        messages = []
        ...
        for message in message_space:
            if not message.att_vecs:
                continue
            if any(
                1 - self.calculate_similarity(agent_att_vec, att_vec) < self.vector_similarity_threshold
                for _, att_vec in message.att_vecs
                for agent_att_vec in agent.att_vecs
            ):
                messages.append(message)
...
class OpinionUpdater():
    ...
    def update(self, agent: VectorAgent, messages: List[Message]):
        ...
        for i in range(self.n_vecs):
            att_update = opinion_updates[i] + vector_updates[i]
            topic: Topic = agent.model.topic_space.get_topic(i)

            new_unnormed = agent.att_vecs[i] + (lam * topic.activation * att_update)
            norm = np.linalg.norm(new_unnormed)
            agent.next_att_vecs[i] = new_unnormed / norm if norm > 0 else agent.att_vecs[i]
        ...
        for message in messages:
            ...
            for topic_id, opinion in message.opinions:
                ...
                if opinion_diff < self.epsilon_T_op:
                    best_index, _ = self._find_extreme_vec_index(agent.att_vecs, topic.vector, self.opinion_assimilative_method)
                    if best_index is not None:
                        update[best_index] += target - agent.att_vecs[best_index]

                elif opinion_diff > self.epsilon_R_op:
                    best_index, _ = self._find_extreme_vec_index(agent.att_vecs, topic.vector, self.opinion_repulsive_method)
                    if best_index is not None:
                        update[best_index] -= target - agent.att_vecs[best_index]
        ...
        for message in messages:
            ...
            for vec_id, att_vec in message.att_vecs:
                ...
                if self.vector_assimilative_method == 'matched':
                    similarity = self.calculate_similarity(att_vec, agent.att_vecs[vec_id])
                    if 1 - similarity < self.epsilon_T_vec:
                        update[vec_id] += att_vec - agent.att_vecs[vec_id]
                    elif similarity < self.epsilon_R_vec:
                        update[vec_id] -= att_vec - agent.att_vecs[vec_id]
                else:
                    best_index, best_similarity = self._find_extreme_vec_index(agent.att_vecs, att_vec, self.vector_assimilative_method)
                    if best_index is not None and 1 - best_similarity < self.epsilon_T_vec:
                        update[best_index] += att_vec - agent.att_vecs[best_index]
                        
                    best_index, best_similarity = self._find_extreme_vec_index(agent.att_vecs, att_vec, self.vector_repulsive_method)
...
class MessageProducer:
    def __init__(self,
                 method: Literal['probabilistic', 'reciprocal', 'similar', 
                                'dissimilar', 'opinionated', 'random_targets', 'broadcast'],
                 similarity_method: SIMILARITY_METHODS,
                 ...
                 include_att_vecs: bool = False,
                 include_pers_vecs: bool = True,
                 n_topics_per_message: int = None,
                 n_att_vecs_per_message: int = None,
                 n_pers_vecs_per_message: int = None,
                 ):
        ...
        self.include_att_vecs = include_att_vecs
        self.include_pers_vecs = include_pers_vecs
        self.n_topics_per_message = n_topics_per_message
        self.n_att_vecs_per_message = n_att_vecs_per_message
        self.n_pers_vecs_per_message = n_pers_vecs_per_message
        
    def set_method(self, method: str):
        match method:
            case 'probabilistic':
                self.method = self.probabilistic
            case 'reciprocal':
                self.method = self.reciprocal
            case 'similar':
                self.method = self.similar
            case 'dissimilar':
                self.method = self.dissimilar
            case 'opinionated':
                self.method = self.opinionated
            case 'random_targets':
                self.method = self.random_targets
            case 'broadcast':
                self.method = self.broadcast
                
    def produce(self, agent) -> List[Message]:
        return self.method(agent)
    
    def _create_message(self, agent, target_ids: List[int] = None) -> Message:
        opinions = None
        att_vecs = None
        pers_vecs = None
        ...
        if self.include_att_vecs:
            all_att_vecs = list(range(len(agent.att_vecs)))
            if self.n_att_vecs_per_message is not None:
                vec_ids = self.rng.choice(all_att_vecs,
                                         min(self.n_att_vecs_per_message, len(all_att_vecs)),
                                         replace=False)
            else:
                vec_ids = all_att_vecs
            att_vecs = [(i, agent.att_vecs[i].copy()) for i in vec_ids]
    ...
    def probabilistic(self, agent) -> List[Message]:
        ...
        n_messages = min(self.n_max_messages, len(all_agents))
        messages = []
        
        for _ in range(n_messages):
            n_to_target = min(self.n_targets, len(all_agents))
            targets = self.rng.choice(all_agents, size=n_to_target, replace=False)
            target_ids = [t.unique_id for t in targets]
            messages.append(self._create_message(agent, target_ids))
        
        return messages
    
    def reciprocal(self, agent) -> List[Message]:
        if not hasattr(agent, 'message_history') or len(agent.message_history) == 0:
            return self.probabilistic(agent)
        ...
        if self.rng.random() < boosted_rate:
            ...
            n_to_target = min(self.n_targets, len(sender_counts))
    ...
    def similar(self, agent) -> List[Message]:
        ...
        if similar_agents:
            n_to_target = min(self.n_targets, len(similar_agents))
    ...
    def dissimilar(self, agent) -> List[Message]:
        ...
        if dissimilar_agents:
            n_to_target = min(self.n_targets, len(dissimilar_agents))
    ...
    def opinionated(self, agent) -> List[Message]:
        if not hasattr(agent, 'att_vecs') or not agent.model.topic_space:
            return self.probabilistic(agent)
    ...
    def broadcast(self, agent) -> List[Message]:
        if self.rng.random() < self.message_rate:
            return [self._create_message(agent, None)]
        return []
...
class VectorAgent(mesa.Agent):
    def __init__(self,
        ...
        self.att_vecs = []
        for _ in range(n_att_vecs):
            vec = self._rng.standard_normal(n_dims)
            vec = vec / np.linalg.norm(vec)
            self.att_vecs.append(vec)
        ...
        self.next_att_vecs = [v.copy() for v in self.att_vecs]
    ...
    def calculate_opinion(self, topic) -> float:
        ...
        for pers_vec in self.pers_vecs:
            avg_vec += pers_vec
        for att_vec in self.att_vecs:
            avg_vec += att_vec
        avg_vec /= (self.n_att_vecs + self.n_pers_vecs)
    ...
    def step(self):
        ...
        if self.model.data_collector and self.model.data_collector.debug_level > 0:
            opinions_before = [(i, self.calculate_opinion(topic)) 
                             for i, topic in enumerate(self.model.topic_space.get_all_topics())]
            att_vecs_before = [v.copy() for v in self.att_vecs]
        
        self.model.opinion_updater.update(self, messages_received)
        
        self.att_vecs = [v.copy() for v in self.next_att_vecs]
        ...
        if self.model.data_collector and self.model.data_collector.debug_level > 0:
            opinions_after = [(i, self.calculate_opinion(topic)) 
                            for i, topic in enumerate(self.model.topic_space.get_all_topics())]
            att_vecs_after = [v.copy() for v in self.att_vecs]
            ...
            self.model.data_collector.record_agent(
                ...
                att_vecs_before=att_vecs_before,
                att_vecs_after=att_vecs_after
            )
